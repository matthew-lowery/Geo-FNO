...
def legendre_poly_grad(n, x):
    ...
    def body(k, val):
        ...
        P_n_plus_1 = ((2*k - 1) * x * P_n - (k - 1) * P_n_minus_1) / k
        return P_n, P_n_plus_1
    P_nm1, P_n = jax.lax.fori_loop(2, n+1, body, (P_n_minus_1, P_n))
    ### n=0->0, n=1->1, n>=2-> n(Pnm1 - xPn) / (1-x^2) 
    return jnp.where(n==0, 
                     jnp.zeros_like(x),
                     jnp.where(n==1, 
                               jnp.ones_like(x),
                               n * (P_nm1 - x * P_n) / (1 - x**2 + jnp.finfo(x.dtype).eps))) # due to div by (1-x**2), v bad at 1 and -1 
...
#### evaluate each poly of a given degree on a given dimension of x
...
def poly_grad_eval(degrees, some_poly, some_poly_grad, x):
    ### for grad wrt to x[i] --> dP/dx[i] * prod for j!=i (P(x[j]))
    def dP_dxi(i):
        out = jnp.ones_like(x[i])
        for j in range(len(degrees)):
            if i == j:
                out *= some_poly_grad(degrees[j], x[j])
            else:
                out *= some_poly(degrees[j], x[j])
        return out
    legendre_grad_values = jnp.stack([dP_dxi(i) for i in range(len(degrees))]) ### 2,2
    return legendre_grad_values
...
def legendre_poly_grad_eval(xs, multi_indices):
    def vandermonde_single_point(x):
        return vmap(lambda degrees: poly_grad_eval(degrees, legendre_poly, legendre_poly_grad, x))(multi_indices)
