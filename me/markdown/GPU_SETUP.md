...
From the Delta login node, with Conda available:

```bash
cd /u/mlowery/Geo-FNO/me
conda env create -f environment.yml
conda activate operator-benchmarks-gpu
python -m pip check
wandb login
```
...
```bash
srun --partition=gpuA100x4 --account=bgcs-delta-gpu --nodes=1 --ntasks=1 --gpus-per-node=1 --cpus-per-task=1 --mem=4G --time=00:05:00 \
...
[`train_forced_recovery_explicit.sh`](../train_forced_recovery_explicit.sh) contains 16 explicit training commands, ordered shortest to longest. It uses the active environment's Python, or `TRAIN_PYTHON` when set. Each GPU job checks CUDA and package imports before training.
