...
class FNO2d(nn.Module):
    ...
    def forward(self, u, code=None, x_in=None, x_out=None, iphi=None):
        ...
        ### for, what if theres an input func? not just shape
        ...
        u = self.fc0(u)
        u = u.permute(0, 2, 1)
        # print(grid.shape, [20, 2, 40, 40] x_in.shape, [20, 972, 2])  u.shape [20, 32, 972])
        uc1 = self.conv0(u, x_in=x_in, iphi=iphi, code=code) # print(uc1.shape) torch.Size([20, 32, 40, 40])
        uc3 = self.b0(grid)
        uc = uc1 + uc3
        uc = F.gelu(uc)
...
class IPHI(nn.Module):
    ...
    def forward(self, x, code=None):
        ...
        if code!= None:
            cd = self.fc_code(code) #### 20,42 ---> 20,32
            cd = cd.unsqueeze(1).repeat(1,xd.shape[1],1) ##### 20, 972, 96 concat 20, 1 (broadcast), 32
