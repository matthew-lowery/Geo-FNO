...
def _format_geometry(
    ...
    if problem in {"taylor_green_time", "taylor_green_time_coeffs"}:
        spatial_points = np.column_stack((points, np.zeros(len(points))))
        output_points = np.repeat(spatial_points, len(TIME_LEVELS), axis=0)
        output_points[:, 2] = np.tile(TIME_LEVELS, len(points))
        if problem.endswith("coeffs"):
            input_points = np.asarray([[0, 1, 0], [0, 0, 0]], dtype=np.float64)
        else:
            input_points = spatial_points
    ...
    if problem == "taylor_green_coeffs":
        input_points = np.asarray([[0, 1], [0, 0]], dtype=np.float64)
